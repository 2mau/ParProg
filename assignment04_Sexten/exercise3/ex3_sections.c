...
void mergesort_parallel_omp(int32_t a[], int32_t size, int32_t temp[], int threads) {
	if(threads == 1) {
	...
	int32_t threadsH = threads / 2;
#pragma omp parallel sections
	{
#pragma omp section
		mergesort_parallel_omp(a, sizeH, temp, threadsH);
#pragma omp section
		mergesort_parallel_omp(a + sizeH, size - sizeH, temp + sizeH, threads - threadsH);
	}
