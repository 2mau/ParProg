...
- Are there any (performance) issues in the given code? Document your findings.  
    The algorithm contains multiple errors and performance can be ompimized by directly computing the sum instead of 
    using the two for loops for combining partial results  
  ...
  ```
  ```cpp
  // local_res false sharing (not initialized)
  ...
  ```cpp
  // res (should be inc in a critical region)
  // omp_get_num_threads is always 1 outside parallel region
	unsigned long res = 0;
...
- Improve the performance of the given source code. 
    ```cpp
    #pragma omp parallel for default(none) shared(n, a, b) reduction(+ : res)
        for(long i = 0; i < n; ++i) {
            for(long j = 0; j < n; ++j) {
                for(long k = 0; k < n; ++k) {
                    res += a[i][k] * b[k][j];
...
The optimizations clearly paid off!
 
| Original | Optimized  |
|---|---|
|  ![O3NoOpt](./exercise2/O3NoOpt.png)   |  ![O3Opt](./exercise2/O3Opt.png)   |
